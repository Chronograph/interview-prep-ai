@php
    $resume = $group['selected_resume'];
    $metrics = $resume->getPerformanceMetrics();
    $versionCount = count($group['versions']);
@endphp
        ...
        @if($versionCount > 1)
            <div class="pt-4 mt-4 border-t border-gray-200">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <!-- Clickable version count dropdown -->
                    <x-dropdown align="right" width="64">
                        <x-slot name="trigger">
                            <button class="flex items-center gap-2 text-xs text-gray-500 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition-colors duration-150">
                        ...
                        @foreach($group['versions'] as $version)
                            <x-dropdown.item
                                wire:click="switchResumeVersion({{ $group['group_id'] }}, {{ $version->id }})"
                                class="flex items-center justify-between w-full"
                            >
                                <div class="flex items-center gap-3">
                                    <div class="flex items-center gap-2">
                                        <span class="font-semibold text-sm">v{{ $version->version }}</span>
                                        @if($version->id === $group['selected_version_id'])
                                            <svg class="w-4 h-4 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                            </svg>
                                        @endif
                                    </div>
                                    <span class="text-xs text-gray-500">{{ $version->updated_at->format('M j, Y') }}</span>
                                </div>
                            </x-dropdown.item>
                        @endforeach
                    </x-dropdown>
                ...
                <div class="flex items-center gap-2">
                    @php
                        $currentVersion = $group['versions']->firstWhere('id', $group['selected_version_id']);
                    @endphp
                    @if($currentVersion)
                        <div class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 text-blue-700 rounded-lg text-sm font-medium">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path>
                            </svg>
                            <span class="font-semibold">v{{ $currentVersion->version }}</span>
                            <span class="text-xs text-blue-600">{{ $currentVersion->updated_at->format('M j, Y') }}</span>
                            <span class="text-xs text-blue-600 bg-blue-200 px-2 py-0.5 rounded-full">Current</span>
                        </div>
                    @endif
                </div>
            </div>
        @else
            <div class="pt-4 mt-4 border-t border-gray-200">
                <div class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 text-blue-700 rounded-lg text-sm font-medium">
                    ...
                    <span class="font-semibold">v{{ $resume->version }}</span>
